"""Alert Processor Service — persists incoming AlertEvents to the database."""
...
class AlertProcessor:
    def process_incoming_alert(self, alert: AlertEvent, db: Session) -> AlertRecord:
        ...
        logger.info(f"Processing incoming alert {alert.alert_id} ({alert.severity}) for trader {alert.trader_id}")
        ...
        logger.info(f"Alert {alert.alert_id} persisted to database (id={record.id})")
        return record
